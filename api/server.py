...
VERIFIER_PATH = os.path.expanduser("~/freshscan/models/fruit_verifier.pth")
...
# Fruits that look similar — allow these as valid matches for each other
SIMILAR_FRUITS = {
    "bellpepper": {"capsicum", "bellpepper"},
    "capsicum": {"capsicum", "bellpepper"},
}
VERIFY_THRESHOLD = 0.30  # Minimum probability for the selected fruit type
...
def load_models():
    ...
    # Load fruit type verifier
    global fruit_verifier, fruit_classes
    if os.path.exists(VERIFIER_PATH):
        v_ckpt = torch.load(VERIFIER_PATH, map_location=DEVICE, weights_only=False)
        v_model = timm.create_model('efficientnet_b0', pretrained=False,
                                     num_classes=len(v_ckpt['classes']))
        v_model.load_state_dict(v_ckpt['model_state_dict'])
        v_model.to(DEVICE).eval()
        fruit_verifier = v_model
        fruit_classes = v_ckpt['classes']
        print(f"✅ Fruit verifier loaded: {fruit_classes} (acc: {v_ckpt.get('val_acc', '?'):.4f})")
    else:
        print(f"⚠️ Fruit verifier not found at {VERIFIER_PATH}")
        print("   Run: python3 models/train_fruit_verifier.py")
        print("   Dropdown verification will be skipped.")
...
# ══════════════════════════════════════════════════════════════
# FRUIT TYPE VERIFICATION
# ══════════════════════════════════════════════════════════════
verifier_transform = transforms.Compose([
    transforms.Resize((224, 224)),
    transforms.ToTensor(),
    transforms.Normalize([0.485, 0.456, 0.406], [0.229, 0.224, 0.225])
])

def verify_fruit_type(image_pil, selected_type):
    """
    Verify that the uploaded image matches the selected fruit type.
    Returns (is_valid, detected_type, confidence, message)
    """
    if fruit_verifier is None:
        # No verifier loaded — skip verification
        return True, selected_type, 1.0, "Verification skipped (verifier not loaded)"

    tensor = verifier_transform(image_pil).unsqueeze(0).to(DEVICE)
    with torch.no_grad():
        logits = fruit_verifier(tensor)
        probs = torch.softmax(logits, dim=1)[0]

    top_idx = probs.argmax().item()
    detected_type = fruit_classes[top_idx]
    detected_conf = float(probs[top_idx])

    # Check if selected type matches detected type
    selected_lower = selected_type.lower()

    # Direct match
    if detected_type == selected_lower:
        return True, detected_type, detected_conf, "Match confirmed"

    # Check similar fruits (e.g., bellpepper/capsicum)
    similar = SIMILAR_FRUITS.get(selected_lower, {selected_lower})
    if detected_type in similar:
        return True, detected_type, detected_conf, "Match confirmed (similar type)"

    # Check if selected type has reasonable probability
    if selected_lower in fruit_classes:
        sel_idx = fruit_classes.index(selected_lower)
        sel_prob = float(probs[sel_idx])
        if sel_prob >= VERIFY_THRESHOLD:
            return True, detected_type, sel_prob, f"Acceptable ({sel_prob:.0%} probability for {selected_type})"

    # Mismatch
    return (False, detected_type, detected_conf,
            f"You selected '{selected_type}' but this looks like '{detected_type}' "
            f"({detected_conf:.0%} confidence). Please check your selection.")
...
async def scan_batch(file: UploadFile = File(...), fruit_type: str = Form(...)):
    ...
    # Verify fruit type matches the image
    is_valid, detected, det_conf, message = verify_fruit_type(image, fruit_type)
    if not is_valid:
        return JSONResponse(status_code=400, content={
            "error": "fruit_type_mismatch",
            "message": message,
            "selected_type": fruit_type,
            "detected_type": detected,
            "detected_confidence": round(det_conf, 3),
        })
...
async def health():
    return {
        ...
        "verifier_loaded": fruit_verifier is not None,
        "yolo_loaded": yolo is not None,
        "img_size": img_size,
        "classes": class_names,
        "fruit_types": fruit_classes,
